...
def eval_model_5_bundle(D_name,Net_name,X_d,y_d,x_test,y_test,cycle_end_idx, number_model=10,idx=None):
    '''
...
def train_model(D_name,Net_name,X_d,y_d,x_train,x_val,y_train,y_val,cycle_end_idx,nb_classes=2, lr_reduce=True, lr_factor = 0.5, earlystopping=True, lr_patience=1, batch_size=64,lr=0.001, max_epochs=50,gpu=[0]):
    '''
    Training the model
    ...
    for i in range(5):
        ...
        classifer = init_model(model_name=Net_name,lr_reduce=lr_reduce, lr_factor = lr_factor, earlystopping=earlystopping, lr_patience=lr_patience, batch_size=batch_size,lr=lr, max_epochs=max_epochs,gpu=gpu,default_root_dir="./model/new_train_lrR_ET_"+D_name+"_"+Net_name+"/",nb_classes=nb_classes)
